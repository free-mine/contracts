...
contract Gate is DepositStore {
  ...
  function completeStage() external onlyOwner {
    uint256 lastStageId = currentStageId - 1;
    Stage storage lastStage = stages[lastStageId];
     ...
     */
    if (!lastStage.ready && lastStage.tokenAmount > 0) {
  ...
  /**
   * @notice Функция не предназначена для вызова до закрытия стадии 1.
   */
